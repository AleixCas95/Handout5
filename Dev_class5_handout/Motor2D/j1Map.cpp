...
void j1Map::Draw()
{
	...
	for (int x = 0; x<data.width;x++)
	{
		for (int y = 0; y < data.height; y++) {
			...
			SDL_Rect rect = item->data->GetTileRect(itemlayer->data->getvaluexy(x,y));
...
bool j1Map::CleanUp()
{
	...
	layer_item = data.maplayers.start;
...
bool j1Map::Load(const char* file_name)
{
	...
	for (layers = map_file.child("map").child("layer"); layers&&ret; layers = layers.next_sibling("layer")) 
	{
		...
		if (ret == true) 
		{
			ret = LoadLayer(layers, layer_data);

		}

		data.maplayers.add(layer_data);
	}
...
bool j1Map::LoadLayer(pugi::xml_node& node, MapLayer* layer)
{
	...
	inline uint MapLayer::getvaluexy(int x, int y)const
 {
